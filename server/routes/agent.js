...
import { queryCopilotSDK } from '../copilot-sdk.js';
import { spawnCursor } from '../cursor-cli.js';
import { queryCodex } from '../openai-codex.js';
import { spawnGemini } from '../gemini-cli.js';
import { Octokit } from '@octokit/rest';
import { CLAUDE_MODELS, CURSOR_MODELS, CODEX_MODELS, COPILOT_MODELS } from '../../shared/modelConstants.js';
 ...
 *
 * @param {string} provider - (Optional) AI provider to use. Options: 'claude' | 'cursor' | 'codex' | 'gemini' | 'copilot'
 *                           Default: 'claude'
 *
...
router.post('/', validateExternalApiKey, async (req, res) => {
  ...
  if (!['claude', 'cursor', 'codex', 'gemini', 'copilot'].includes(provider)) {
    return res.status(400).json({ error: 'provider must be "claude", "cursor", "codex", "gemini", or "copilot"' });
  }
  ...
  try {
    ...
    if (provider === 'claude') {
      ...
      console.log('🤖 Starting GitHub Copilot SDK session');

      await queryCopilotSDK(message.trim(), {
        projectPath: finalProjectPath,
        cwd: finalProjectPath,
        sessionId: sessionId || null,
        model: model || COPILOT_MODELS.DEFAULT,
        permissionMode: 'bypassPermissions'
      }, writer);
    }
